    ...
    <div className='p-8 bg-black min-h-screen text-slate-200'>
      <header className='mb-10 flex justify-between items-end'>
        ...
        <div className='flex gap-4'>
          <DateRangeSelector />
          <button className='bg-emerald-600 px-4 py-2 rounded-lg font-bold text-sm hover:bg-emerald-500 transition-all'>
            Export CSV
          </button>
        </div>
      </header>
      ...
      <div className='grid grid-cols-12 gap-8'>
          ...
          <div className='flex justify-between items-center mb-6'>
            ...
            <div className='flex gap-2 text-xs font-mono'>
              <span className='flex items-center gap-1 text-emerald-400'>
                <div className='w-2 h-2 rounded-full bg-emerald-400' /> Equity
              </span>
              <span className='flex items-center gap-1 text-rose-500'>
                <div className='w-2 h-2 rounded-full bg-rose-500' /> Drawdown
              </span>
            </div>
          </div>
          <div className='h-80'>
        ...
        {/* Side Panel: Directional Bias */}
        <div className='col-span-12 lg:col-span-4 space-y-8'>
          <div className='bg-slate-900/40 border border-slate-800 rounded-2xl p-6'>
            <h3 className='font-bold mb-4'>Long / Short Ratio</h3>
            <div className='flex items-center justify-between mb-2 text-sm font-mono'>
              <span className='text-emerald-400'>LONG 65%</span>
              <span className='text-rose-400'>SHORT 35%</span>
            </div>
            <div className='w-full h-4 bg-rose-500/20 rounded-full overflow-hidden flex'>
              <div className='h-full bg-emerald-500' style={{ width: '65%' }} />
            </div>
            <p className='mt-4 text-xs text-slate-500 italic'>
              "You perform 15% better on Long positions in this timeframe."
            </p>
          </div>

          <div className='bg-slate-900/40 border border-slate-800 rounded-2xl p-6'>
            <h3 className='font-bold mb-4 text-sm uppercase text-slate-400'>
              Top Tradable Assets
            </h3>
            <div className='space-y-4'>
              <AssetRow symbol='SOL-PERP' pnl={4200} count={12} />
              <AssetRow symbol='BONK-PERP' pnl={-120} count={5} />
              <AssetRow symbol='JUP-OPTIONS' pnl={850} count={3} />
            </div>
          </div>
        </div>
      </div>
